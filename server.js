...
async function initDB() {
  try {
    ...
    dbCache = JSON.parse(JSON.stringify(SEED));
    await fsp.writeFile(DB_FILE, JSON.stringify(dbCache, null, 2));
    console.log('✦ Database seeded at', DB_FILE);
  }
}

// ── Write back async, debounced — never blocks a request ─────────────────────
function scheduleWrite() {
  if (writeQueued) return;
  writeQueued = true;
  setImmediate(async () => {
    try {
      dbCache.meta.lastSync = new Date().toISOString();
      await fsp.writeFile(DB_FILE, JSON.stringify(dbCache, null, 2));
    } catch (e) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === 'POST' && parts[0] === 'transactions') {
    const body = await parseBody(req);
    const txn = { id: `txn-${Date.now()}`, timestamp: new Date().toISOString(), ...body };
